...
public class LichHenRepository
{
    ...
    public async Task<ReportSummaryDto> GetReportSummaryAsync()
    {
        ...
        const string dailySql = """
            SELECT TOP 14 CONVERT(varchar(10), NgayHen, 23) AS NgayHen, COUNT(1) AS SoLich
            FROM LichHen
            GROUP BY NgayHen
            ORDER BY NgayHen DESC;
            """;
        const string monthlySql = """
            SELECT YEAR(NgayHen) AS Nam, MONTH(NgayHen) AS Thang, COUNT(1) AS SoLich
            FROM LichHen
            GROUP BY YEAR(NgayHen), MONTH(NgayHen)
            ORDER BY Nam DESC, Thang DESC;
            """;
        const string topServiceSql = """
            SELECT TOP 5 dv.MaDichVu, dv.TenDichVu, COUNT(lhdv.MaLichHen) AS SoLanDat
            FROM DichVu dv
            LEFT JOIN LichHenDichVu lhdv ON lhdv.MaDichVu = dv.MaDichVu
            GROUP BY dv.MaDichVu, dv.TenDichVu
            ORDER BY SoLanDat DESC, dv.TenDichVu;
            """;
        const string overviewSql = """
            SELECT
                ISNULL((SELECT SUM(TongTien) FROM HoaDon WHERE TrangThaiThanhToan = 'Paid'), 0) AS TongDoanhThu,
                (SELECT COUNT(1) FROM NguoiDung WHERE VaiTro = 'Customer') AS TongKhachHang,
                (SELECT COUNT(1) FROM NguoiDung WHERE VaiTro = 'Customer'
                    AND YEAR(NgayTao) = YEAR(GETDATE()) AND MONTH(NgayTao) = MONTH(GETDATE())) AS KhachHangMoiThangNay;
            """;
        const string staffSql = """
            SELECT nd.MaNguoiDung AS MaNhanVien, nd.HoVaTen,
                   COUNT(lh.MaLichHen) AS SoLichDuocGiao,
                   SUM(CASE WHEN lh.TrangThai = 'Completed' THEN 1 ELSE 0 END) AS SoLichHoanThanh,
                   ISNULL(SUM(CASE WHEN hd.TrangThaiThanhToan = 'Paid' THEN hd.TongTien ELSE 0 END), 0) AS DoanhThu
            FROM NguoiDung nd
            LEFT JOIN LichHen lh ON lh.MaNhanVien = nd.MaNguoiDung
            LEFT JOIN HoaDon hd ON hd.MaLichHen = lh.MaLichHen
            WHERE nd.VaiTro = 'Staff'
            GROUP BY nd.MaNguoiDung, nd.HoVaTen
            ORDER BY SoLichHoanThanh DESC, nd.HoVaTen;
            """;
        var overview = await connection.QuerySingleAsync(overviewSql);

        return new ReportSummaryDto
        {
            LichTheoNgay = await connection.QueryAsync<DailyBookingReportDto>(dailySql),
            LichTheoThang = await connection.QueryAsync<MonthlyBookingReportDto>(monthlySql),
            TopDichVu = await connection.QueryAsync<TopServiceReportDto>(topServiceSql),
            TongDoanhThu = Convert.ToDecimal(overview.TongDoanhThu),
            TongKhachHang = Convert.ToInt32(overview.TongKhachHang),
            KhachHangMoiThangNay = Convert.ToInt32(overview.KhachHangMoiThangNay),
            HieuSuatNhanVien = await connection.QueryAsync<StaffPerformanceDto>(staffSql)
        };
    }
}
