...
export function ProfilePage({ session, pets, onSessionChanged, onPetsChanged }: ProfilePageProps) {
  ...
  if (session.user.vaiTro !== 'Customer') {
    return <Navigate replace to={session.user.vaiTro === 'Admin' ? '/admin' : '/staff'} />;
  }
  ...
  return (
    <div className="profile-page">
      ...
      <section className="profile-pets-section">
        ...
        <div className="profile-pets-layout">
          <div className="profile-pet-list">
            {pets.map((pet) => (
              <article className={editingPetId === pet.maThuCung ? 'profile-pet-card profile-pet-card--active' : 'profile-pet-card'} key={pet.maThuCung}>
                <div className="profile-pet-card__avatar" aria-hidden="true">{getPetInitial(pet)}</div>
                <div className="profile-pet-card__body">
                  <div>
                    <h3>{pet.tenThuCung}</h3>
                    <p>{pet.loaiThuCung} · {pet.giong}</p>
                  </div>
                  <dl>
                    <div><dt>Giới tính</dt><dd>{pet.gioiTinh || 'Chưa cập nhật'}</dd></div>
                    <div><dt>Ngày sinh</dt><dd>{pet.ngaySinh || 'Chưa cập nhật'}</dd></div>
                    <div><dt>Cân nặng</dt><dd>{pet.canNang ? `${pet.canNang} kg` : 'Chưa cập nhật'}</dd></div>
                  </dl>
                  {pet.ghiChu ? <p className="profile-pet-card__note">{pet.ghiChu}</p> : null}
                </div>
            ...
            {pets.length === 0 ? <p className="empty-state">Chưa có thú cưng nào. Hãy tạo hồ sơ đầu tiên ở biểu mẫu bên cạnh.</p> : null}
          </div>
